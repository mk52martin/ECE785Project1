...
int main(int argc, char * argv []) {
  int fd;
  unsigned int level_in, impulse_width=10;
  unsigned int level_was_0=0;
  ...
  if (argc == 2) {
    ...
    if ((res != 1) || (val < 0))
      printf("Ignoring illegal impulse width request: %s\n", argv[1]);
    else
      impulse_width = (unsigned int) val;
  }
  printf("Impulse width set to %d itsy-seconds\n", impulse_width);
  ...
  while (run) {
	...
	for (int i = 1; i < impulse_width; i++)
	  gpio->GPSET[0] = MASK(OUTPUT_POS); // Keep the LED on
	level_was_0 = 0;
